...
The current baseline was trained and evaluated locally on the real WM-811K file using its official split: 54,355 training wafers and 118,595 test wafers. Unlabelled records were excluded from supervised training and evaluation.
...
Accuracy is not treated as the headline metric: 110,701 of the 118,595 test wafers are labelled `none`. Macro-F1 and per-class recall expose the baseline's weak performance on rare spatial patterns. See [`docs/MODEL_RESULTS.md`](docs/MODEL_RESULTS.md) for the class-level results, confusion-matrix analysis and the next modelling hypothesis.
...
Then open `http://localhost:8000`. Without the dataset, the application intentionally starts in `demo` mode using visibly labelled simulated wafer maps. API documentation is available at `http://localhost:8000/docs`.
...
| `GET /api/models/baseline/evaluation` | Held-out aggregate and per-class evaluation |
| `GET /api/wafers/{wafer_id}/prediction` | Predicted class, confidence and ground-truth comparison |
...
Model metrics are written to `artifacts/metrics.json` and the fitted pipeline to `artifacts/baseline.joblib`. Metrics are generated from the actual local dataset and are never hard-coded.

The model artifact now stores the evaluation contract used by the dashboard: aggregate metrics, per-class precision/recall/F1, test support, class labels and the confusion matrix. If you update the training code, rerun training before using the new evaluation endpoint.
...
The command center treats a labelled wafer as a reviewable model decision: **ground truth -> prediction -> confidence -> correct/incorrect**. This is evaluation evidence, not a claim that the model would be deployed without human review.

Similarity is computed from standardized engineered wafer-map features and is presented as case retrieval, not proof that two wafers share a physical root cause. Investigation output is deterministic decision support grounded in the observed map and retrieved cases; it does not invent equipment telemetry, maintenance history or intervention outcomes absent from WM-811K.
